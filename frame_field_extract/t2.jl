...
# --- Visualization ---
function plot_cut_graph(topo, cut_edges, filename)
    fig = Figure(size=(1000, 800))
    ax = Axis(fig[1, 1], aspect=DataAspect(), title="Cut Graph (Red)")
    
    # 1. Plot all mesh edges (faint gray)
    xs, ys = Float64[], Float64[]
    for tri in topo.faces
        p1, p2, p3 = topo.vertices[tri[1]], topo.vertices[tri[2]], topo.vertices[tri[3]]
        push!(xs, p1.x, p2.x, NaN, p2.x, p3.x, NaN, p3.x, p1.x, NaN)
        push!(ys, p1.y, p2.y, NaN, p2.y, p3.y, NaN, p3.y, p1.y, NaN)
    end
    lines!(ax, xs, ys, color=(:gray, 0.3), linewidth=0.5)
    
    # 2. Plot Cut Edges (Thick Red)
    ...
    lines!(ax, cx, cy, color=:red, linewidth=2.0)
    
    # 3. Plot Boundaries (Blue points) for context
    # (Optional, reusing boundary logic inside if needed, or just skipping)
    
    save(filename, fig)
    println("Saved cut graph visualization to $filename")
end
