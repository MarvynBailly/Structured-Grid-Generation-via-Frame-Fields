...
\end{equation}
where $\alpha_k$ is the angle at vertex $v$ in face $f_k$. Ray et. al. (2008b) shows that vertices with non-zero index correspond to singularities in the frame field which will later correspond to irregular vertices in the quadrilateral mesh. The index is ensured to be an multiple of $1/4$ and will determine the valence of the irregular vertex in the quad mesh. It can be shown that an index of $+1/4$ corresponds to a vertex with valence $3$ while an index of $-1/4$ corresponds to a vertex with valence $5$. In general, we have the relationship
\begin{equation}
